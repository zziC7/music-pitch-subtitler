...
def generate_srt(midi_obj, output_srt):
    """从 PrettyMIDI 对象提取人声音轨并生成 SRT 字幕"""
    ...
    notes = sorted(target_instrument.notes, key=lambda x: x.start)
    
    with open(output_srt, 'w', encoding='utf-8') as f:
        for i, note in enumerate(notes):
            if note.end - note.start < 0.05: continue
            
            start = format_time_srt(note.start)
            end = format_time_srt(note.end)
            pitch = midi_number_to_name(note.pitch) # 假设你已经定义了此函数
